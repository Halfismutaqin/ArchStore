                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="checkout__form__input">
                                    ...
                                    <input type="text" name="phone_number" value="{{ $data['myAccount']->phone ?? '' }}" required>
                                </div>
                            </div>
                                    ...
                                    <select name="province_id" id="province_id" class="select-2" required>
                                        ...
                                        @foreach ($data['provinces'] as $province)
                                            <option value="{{ $province['province'] }}"
                                                ...
                                                {{ optional($data['myAccount'])->province_id == $province['province_id'] ? 'selected' : '' }}>
                                                {{ $province['province'] }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6">
                                <div class="checkout__form__input">
                                    <p>City <span>*</span></p>
                                    <input type="hidden" name="get_city" id="get_city"
                                        value="{{ $data['myAccount']->city_id ?? '' }}">
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="checkout__form__input">
                                    ...
                                    <textarea name="address_detail" id="address_detail" rows="4" class="form-control" style="height: 72px"
                                        placeholder="Isikan alamat lengkap, patokan atau gedung" required>{{ $data['myAccount']->address ?? '' }}</textarea>
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadCities(provinceId) {
                var provinceId = $('#province_id option:selected').data('id');
                var getCityId = $('#get_city').val(); // notice the parentheses
                    ...
                    success: function(data) {
                        if (data) {
                            ...
                            $('select[name="city_id"]').append(
                                'option value="" selected>-- Select City --</option>');
                            $.each(data, function(key, city) {
                                var selected = (getCityId == city.city_id) ? ' selected' : '';
                                $('select[name="city_id"]').append('<option value="' + city
                                    .city_name + '" data-id="' + city.city_id + '"' +
                                    selected + '>' + city.type + ' ' + city.city_name +
                                    '</option>');
                            });
                            checkCost();
                        } else {
            ...
            var initialProvince = $('#province_id').val();
            if (initialProvince) {
